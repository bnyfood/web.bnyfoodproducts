class UI_MORECONTENT {
  addToList(list_data) {
    ...
    var url_edit = hostname_site+"/"+"store_manage/location/location_edit_form/"+list_data.store_location_id;
    var url_del = hostname_site+"/"+"store_manage/location/del_action/"+list_data.store_location_id;
    ...
    // Insert cols
    row.innerHTML = `
    <td>${list_data.store_location_name}</td>
...
function loadcontent(offset,sortby,sorttype,is_new_load){
  ...
  $('#spinner-div').show();
	...
	var urls = hostname_site+"/"+"store_manage/location/loaddata_more_ajax";
	      $.ajax({
	        type: "POST",
	        url: urls,
	        data:  {location_search: location_search,offset:offset,sortby:sortby,sorttype:sorttype}, 
	        dataType: "json",
