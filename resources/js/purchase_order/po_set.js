...
$(document).ready(function() {
  ...
  $("#po_make_btn").click(function(event) {
    ...
    const urls_data_po = hostname_site+"/"+"purchase_order/sent_data_make_po";
    //$('#spinner-div').show();
    //var ran_num_pocode_make = $("#ran_num_pocode").val();

    const fd = new FormData(document.forms['product_search']); 

    fd.append('web_supplier_id', $('#web_supplier_id').val());
    fd.append('ran_num_pocode', $('#ran_num_pocode').val());
    fd.append('arr_po_add', JSON.stringify(arr_mat_json));
...
$(document).on('input','[name="po_all_price[]"]', function() {
...
function po_print(po_number){
  ...
  var urls = hostname_site+"purchase_order/print_po/"+po_number+"_"+web_supplier_id;
  //alert(urls);
...
function clear_data_po(){
  ...
  document.getElementById("table_sum_po_price").style.display = "none";
...
function peocess_move(e,obj){
   ...
   // left arrow
    if(tbl_col_length > 0){
      if(table_point_col > 0){
        ...
        for (let x = table_point_col-1; x >= 0; x--) {

          const table_point = table_point_row+"_"+x;
          const type_input = $("#"+table_point).attr('type');
          if(( type_input == "text")||(type_input == "button")){
            table_point_col = x;
            break;
          }
        }
    ...
    // right arrow
    if(tbl_col_length > 0){
      if(table_point_col < tbl_col_length){

        for (let x = table_point_col+1; x <= tbl_col_length; x++) {
    ...
    //console.log(obj.getAttribute("idval"));
    const type_input = $("#"+table_point_row+"_"+table_point_col).attr('type');
    if(type_input == "button"){
      del_data_and_json(obj.getAttribute("idval"));
    }
  }
  ...
  const new_table_point = table_point_row+"_"+table_point_col;

  //console.log("new>"+new_table_point);

  console.log("new>>>"+new_table_point);
  document.getElementById(new_table_point).focus();
...
class UI_supmandata {
  ...
  addToList(data,arr_suppliers,web_material_id) {
    ...
    if (var_price === undefined){
      ...
      var sty_price = "";
      //console.log(">>"+arr_prices[arr_supplier.supplier_name]['last_change_min']);
      if(parseInt(arr_prices[arr_supplier.supplier_name]['last_change_min']) < 30){
       sty_price =  "style='background-color:yellow'";
      }

      td_price = td_price + `<td id='${id_price}'><input type='text' ${sty_price} name='po_all_price[]' id='${id_price_val}' value='${var_price}' class='edittable form-control' onkeydown="peocess_move(event,this)";></td>`;
    ...
    ${td_sum}
    <td>
      <button type="button" id="${id_btn}" idval="${id_del}" class="btn btn-dark icon wb-trash" style="font-size:1em;" value="del_material" title="Delete" onkeydown="peocess_move(event,this)";>
      </button>
    </td>
    `;
...
document.getElementById("del_material_btn").addEventListener("click", function (e) {
 ...
 // alert($(this).index());
  ...
  del_data_and_json(edit_id);
  
});

function del_data_and_json(id){

  if (id !== null){

    const arr_val_id = id.split("_");
    const id_row_del = arr_val_id[1];
    const web_material_id = arr_val_id[2];

    document.getElementById("tr_mat_"+web_material_id).style.display = "table-row";
...
document.getElementById("list-sup-data").addEventListener("click", function (e) {
  ...
  edit_id = e.target.getAttribute("idval");
