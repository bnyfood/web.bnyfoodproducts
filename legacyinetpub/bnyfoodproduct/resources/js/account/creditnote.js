...
function set_search(sel){
  ...
  if(sel.value == '1'){
    ...
    document.getElementById("button_search").style.display = "flex";
...
$('#searchcopy').click(function(){
  ...
  var urls = 'http://'+window.location.hostname+"/adminbny/accounting/print_cn/"+platform+"/"+ordernumber+"/"+daterange+"/copy/"+search_type;
  //alert(urls);
...
$('#search').click(function(){
  ...
  var urls = hostname_site+"accounting/creditnote/creditnote_make/"+platform+"/"+ordernumber+"/"+daterange+"/original/"+search_type;
  //alert(urls);
  return window.open(urls, '', 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width='+w+', height='+h+', top='+top+', left='+left);
