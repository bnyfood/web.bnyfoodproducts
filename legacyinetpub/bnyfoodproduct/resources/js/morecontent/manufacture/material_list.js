class UI_MORECONTENT {
  addToList(list_data) {
    ...
    var url_edit = hostname_site+"/"+"manufacture/brand/brand_edit_form/"+list_data.material_map_supplier_history_id;
    var url_del = hostname_site+"/"+"manufacture/brand/del_action/"+list_data.web_material_id;
    ...
    row.innerHTML = `
      ...
      <td>${list_data.material_unit_price}</td>
...
function loadcontent(offset,sortby,sorttype,is_new_load){
	...
	var urls = hostname_site+"/"+"manufacture/material/loaddata_more_ajax";
  $('#spinner-div').show();
	      $.ajax({
	        type: "POST",
	        url: urls,
