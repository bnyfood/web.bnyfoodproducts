class UI_MORECONTENT {
  addToList(list_data) {
    ...
    var url_edit = hostname_site+"/"+"store_manage/shelf/shelf_edit_form/"+list_data.store_shelf_id;
    var url_del = hostname_site+"/"+"store_manage/shelf/del_action/"+list_data.store_shelf_id;
    ...
    // Insert cols
    row.innerHTML = `
    <td>${list_data.store_shelf_name}</td>
...
function loadcontent(offset,sortby,sorttype,is_new_load){
	...
	var shelf_search = document.getElementById("shelf_search").value;
	//alert(brand_search);

  $('#spinner-div').show();

	var urls = hostname_site+"/"+"store_manage/shelf/loaddata_more_ajax";
	      $.ajax({
	        type: "POST",
	        url: urls,
	        data:  {shelf_search: shelf_search,offset:offset,sortby:sortby,sorttype:sorttype}, 
	        dataType: "json",
