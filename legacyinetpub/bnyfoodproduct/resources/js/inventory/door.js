$("#door_active").change(function(event){

var door_val = "";

//alert(door_val);

if ($('input[name="door_active"]').prop('checked')) {
	door_val = 1;
	door_send_val(door_val);
...
function door_send_val(door_val){

	var arr_path = window.location.pathname.split("/");
	var urls = 'https://'+window.location.hostname+"/"+arr_path[1]+"/"+"door/door_active";

    $.ajax({
	    type: "POST",
	    url: urls,
	    data:  {door_val: door_val}, 
	    dataType: "json",
	    complete: function () {
	    	console.log(door_val);

	    	getLocation();

	    	if(door_val == "1"){
  				document.getElementById("door_txt").innerHTML = "ประตูเปิด";
  			}else{
  				document.getElementById("door_txt").innerHTML = "ประตูปิด";
  			}
	    }
	  });
...
function showPosition(position) {
	/*const x = document.getElementById("location_txt");
  x.innerHTML = "Latitude: " + position.coords.latitude + 
  "<br>Longitude: " + position.coords.longitude;*/

  //var gps1 = new GPS(position.coords.latitude, position.coords.longitude);
//var gps2 = new GPS(7.921512070888384, 98.39491443250235);

  //alert(findDistance(gps1, gps2) + ' meter');

  alert(getDistanceFromLatLonInKm(position.coords.latitude, position.coords.longitude,7.893894378196623,98.36799764918734));
}
