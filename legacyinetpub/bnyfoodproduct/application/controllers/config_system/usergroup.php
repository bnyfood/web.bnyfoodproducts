...
class Usergroup extends Auth_Controller
{
	...
	private function extract_menu_by_name(&$menus, $menu_name)
	{
		...
		foreach ($menus as $idx => &$menu) {
			if (!empty($menu['menu_name']) && $menu['menu_name'] === $menu_name) {
				$found = $menu;
				unset($menus[$idx]);
			...
			if (!empty($menu['submenus']) && is_array($menu['submenus'])) {
				$found = $this->extract_menu_by_name($menu['submenus'], $menu_name);
				if (!empty($found)) {
					return $found;
				}
			}

			if (!empty($menu['lv3_submenus']) && is_array($menu['lv3_submenus'])) {
				$found = $this->extract_menu_by_name($menu['lv3_submenus'], $menu_name);
				if (!empty($found)) {
					return $found;
				}
			}
	...
	private function move_reward_menu_to_crm_lv3($menus)
	{
		if (empty($menus) || !is_array($menus)) {
			return $menus;
		}

		$reward_menu = $this->extract_menu_by_name($menus, 'จัดการรางวัล');
		if (empty($reward_menu)) {
			return $menus;
		}

		$crm_found = false;
		foreach ($menus as &$lv1_menu) {
			if (empty($lv1_menu['submenus']) || !is_array($lv1_menu['submenus'])) {
				continue;
			}

			foreach ($lv1_menu['submenus'] as &$lv2_menu) {
				if (empty($lv2_menu['menu_name']) || $lv2_menu['menu_name'] !== 'CRM') {
					continue;
				}

				if (empty($lv2_menu['lv3_submenus']) || !is_array($lv2_menu['lv3_submenus'])) {
					$lv2_menu['lv3_submenus'] = array();
				}

				$is_exists = false;
				foreach ($lv2_menu['lv3_submenus'] as $lv3_menu) {
					if (
						(!empty($lv3_menu['menu_id']) && !empty($reward_menu['menu_id']) && (string)$lv3_menu['menu_id'] === (string)$reward_menu['menu_id'])
						|| (!empty($lv3_menu['menu_name']) && $lv3_menu['menu_name'] === 'จัดการรางวัล')
					) {
						$is_exists = true;
						break;
					}
				...
				if (!$is_exists) {
					$lv2_menu['lv3_submenus'][] = $reward_menu;
				}
				$crm_found = true;
				break;
			}
		...
		// fallback: if CRM not found, keep menu at root to avoid loss
		if (!$crm_found) {
			$menus[] = $reward_menu;
		}
	...
	function usergroup_manage(){
		...
		$id_en = $this->uri->segment(4);
		$shop_id = $this->session->userdata(SESSION_PREFIX.'shop_id');
		$arr_list_users = $this->user_model->get_user_by_shop_id($shop_id);
		...
		if (!empty($data_groupmenus['Data']['data_menus']) && is_array($data_groupmenus['Data']['data_menus'])) {
			$data_groupmenus['Data']['data_menus'] = $this->move_reward_menu_to_crm_lv3($data_groupmenus['Data']['data_menus']);
		}
