...
class Bnyreward extends CI_Controller
{
    ...
    /**
     * แสดงรูปของรางวัล (โฟลเดอร์เดียวกับ bnyadminreward) — ไม่ต้อง login admin
     * URL: bnyreward/gift_pic/{filename}
     */
    ...
    public function bny_checkprize()
    {
        ...
        $data = array(
            ...
            'gift_lasted_url' => base_url('marketing/crm/bnyadminreward/get_gift_lasted'),
        );
    ...
    public function bny_result()
    {
        ...
        $prize_gift = $is_current_user_winner
            ? $this->get_reward_gift_display($latest_reward)
            : null;
        $data = array(
            'logo_url' => base_url('resources/images/logo_700.png'),
            'customer_name' => $profile['web_user_name'],
            ...
            'prize_gift_pic_url' => !empty($prize_gift['pic_url']) ? $prize_gift['pic_url'] : '',
            'prize_gift_detail' => !empty($prize_gift['detail']) ? $prize_gift['detail'] : '',
    ...
    public function reward_random()
    {
        ...
        if (!empty($existing_reward)) {
            $this->json_out(array(
                ...
                'web_bny_gift_id' => isset($existing_reward['web_bny_gift_id']) ? (int) $existing_reward['web_bny_gift_id'] : null,
        ...
        $current_gift = $this->web_bny_gift_model->select_current();
        $gift_id = !empty($current_gift['web_bny_gift_id']) ? (int) $current_gift['web_bny_gift_id'] : 0;
        if ($gift_id <= 0) {
            $this->json_out(array(
                'status' => false,
                'inserted' => false,
                'message' => 'ไม่พบรางวัลที่ใช้งาน (web_bny_gift_now = 1)',
                'period_start' => $period_start,
                'period_stop' => $period_stop,
            ), 500);
            return;
        ...
        $this->json_out(array(
            ...
            'web_bny_gift_id' => isset($stored_reward['web_bny_gift_id']) ? (int) $stored_reward['web_bny_gift_id'] : $gift_id,
    ...
    /** โฟลเดอร์รูปรางวัล — เดียวกับ marketing/crm/bnyadminreward */
    private function gift_pic_storage_path()
    {
        return APP_STORE_PATH . '/uploads/reward/';
    }

    /** URL รูปรางวัลสำหรับหน้าลูกค้า (ไม่ผ่าน admin auth) */
    private function build_reward_gift_pic_url($pic)
    {
        $pic = trim((string) $pic);
        if ($pic === '') {
            return '';
        ...
        return base_url('bnyreward/gift_pic/' . rawurlencode($filename));
    }

    /**
     * ข้อมูลรางวัลตาม web_bny_gift_id ใน web_bny_reward (อ่านจาก temporal ณ เวลาออกรางวัล)
     */
    private function get_reward_gift_display($reward_row)
    {
        if (empty($reward_row)) {
            return null;
        }
        ...
        $gift = null;
        $reward_id = !empty($reward_row['web_bny_reward_id']) ? (int) $reward_row['web_bny_reward_id'] : 0;
        if ($reward_id > 0) {
            $gift = $this->web_bny_gift_model->select_for_reward_id($reward_id);
        }
        if (empty($gift) && !empty($reward_row['web_bny_gift_id']) && !empty($reward_row['cdate'])) {
            $gift = $this->web_bny_gift_model->select_by_id_at_time(
                (int) $reward_row['web_bny_gift_id'],
                (string) $reward_row['cdate']
            );
        }
        if (empty($gift) && !empty($reward_row['cdate'])) {
            $gift = $this->web_bny_gift_model->select_active_gift_at_time($reward_row['cdate']);
        }
        if (empty($gift)) {
            return null;
        }
        ...
        return array(
            'pic_url' => $this->build_reward_gift_pic_url(isset($gift['web_bny_gift_pic']) ? $gift['web_bny_gift_pic'] : ''),
            'detail' => trim((string) (isset($gift['web_bny_gift_detail']) ? $gift['web_bny_gift_detail'] : '')),
        );
    }
    ...
    private function format_prize_winner_display_for_user($login_id, $profile_phone, $winner_phone_raw)
    {
        ...
        return $this->mask_phone_last4($winner_phone_raw);
    }

    private function mask_phone_last4($phone)
    {
        ...
        return mb_substr($phone, 0, $len - 4) . 'XXXX';
    }
