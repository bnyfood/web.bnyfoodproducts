...
class Web_user_login_model extends CI_Model
{
    ...
    function select_by_web_user_phone($phone)
    {
        if ($phone === null || $phone === '') {
            return null;
        }
    ...
    function select_by_phone_flexible($phones)
    {
        ...
        $facebook_expr = $this->normalized_phone_sql('facebook_phone');
        $google_expr = $this->normalized_phone_sql('google_phone');
        $web_expr = $this->normalized_phone_sql('web_user_phone');
        $in_sql = '(' . implode(',', $escaped) . ')';

        $this->db->select('*');
        $this->db->from('web_user_login');
        $this->db->group_start();
        $this->db->where($facebook_expr . ' IN ' . $in_sql, null, false);
        $this->db->or_where($google_expr . ' IN ' . $in_sql, null, false);
        $this->db->or_where($web_expr . ' IN ' . $in_sql, null, false);
        $this->db->group_end();
        $this->db->order_by('web_user_login_id', 'DESC');
        $query = $this->db->get();
        return $query->row_array();
