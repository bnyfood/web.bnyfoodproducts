...
class Store_shelf_model extends CI_Model
{
    ...
    function insert($data){
    	$this->db->insert('store_shelf', $data);
    	$insert_id = $this->db->insert_id();
    	return $insert_id;
	}
	
	function update($data,$api_id){
    	$this->db->where('store_shelf_id',$api_id);
		$this->db->update('store_shelf',$data);
	}

	function delete($id){
		$this->db->where('store_shelf_id',$id);
		$this->db->delete('store_shelf');
	}
	...
	function select_by_shop_id_search($shopid,$brand_search,$per_page,$offset,$sortby,$sorttype){
		...
		if($brand_search != ""){
			$this->db->like('store_shelf_name',$brand_search);
		}
		if($sortby != ""){
	...
	function select_by_sub($shelf_id){

		$this->db->select('*');
		$this->db->from('store_sub_shelf');
		$this->db->where('store_shelf_id',$shelf_id);
		$query = $this->db->get();
		return $query->result_array();
