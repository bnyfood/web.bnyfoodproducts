...
class Data_prepare_bl
{
	...
	function __construct() 
	{
		...
		$this->CI->load->model('web_shop_model');
	...
	function setdata_register($user_id,$shop_name,$customer_code,$user_lv)
	{
		$success = false;
		$shop_id = $this->create_shop($shop_name,$customer_code);
		if(!empty($shop_id)){
			$group_id = $this->create_group($shop_name,$shop_id,$customer_code,$user_id);
			$this->create_group_tmp($shop_name,$shop_id,$customer_code);
			if(!empty($group_id)){
				$this->user_map_group($group_id,$user_id,$user_lv);
	...
	function setdata_register_user($user_id,$usergroup_id_en,$user_lv)
	{
		$success = false;
		
		if($usergroup_id_en != ""){
			$usergroup_id = $this->CI->encryption_util->decrypt_ssl($usergroup_id_en);
			
			$arr_group = $this->CI->user_group_model->select_by_id($usergroup_id);
			if(!empty($arr_group)){
				$group_id = $arr_group['usergroup_id'];
				$this->create_group_user($group_id,$user_id);
				$this->user_map_group($group_id,$user_id,$user_lv);			
				$success = true;
			}
		}
	...
	function create_group_user($group_id,$user_id){
	...
	function create_group_tmp($shop_name,$shop_id,$customer_code){

		$group_name = "Temp ".$shop_name;
	...
	function user_map_group($group_id,$user_id,$user_lv){
