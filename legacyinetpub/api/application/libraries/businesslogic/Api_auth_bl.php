...
class Api_auth_bl{
	...
	function check_api_auth_token(){

		//redirect(base_url().'users/permission','refresh');


		$arr_header = $this->get_header();

	    $chk_auth = $this->authen_token($arr_header['api_token']);

	    //echo $arr_header['api_token'];
	    ...
	    if($chk_auth['Status'] == "Success"){
	    	...
	    	$chk_auth = $this->CI->json_util->json_unicode($chk_auth);

	    	echo $chk_auth;

	    	//$this->CI->session->set_flashdata('api_status',$chk_auth);
	...
	function create_token_id($user_login_id){

		$ip = $this->get_client_ip();
		$authenid = $user_login_id;
		$datenow = date('Y-m-d h:i:s');
		$makeencode =  $ip.'\|/'.$datenow.'\|/'.API_TOKEN_KEY.'\|/'.$authenid;
		$key_encript = $this->CI->encryption_util->encrypt_decrypt('encrypt',$makeencode,API_TOKEN_KEY2,API_TOKEN_KEY3);
