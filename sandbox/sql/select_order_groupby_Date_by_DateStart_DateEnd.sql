...
-- =============================================
CREATE PROCEDURE [dbo].[select_order_groupby_Date_by_DateStart_DateEnd]
...
[status] as status1,
[status] as status2,
lazada_orders_view.order_number as order_number,
...
where convert(date,convert(varchar, lazada_orders_view.created_at, 23))>=@DateStart and convert(date,convert(varchar, lazada_orders_view.created_at, 23))<=@DateEnd
and lazada_orders_view.[status] in ('packed')
order by lazada_orders_view.created_at
