...
// create by big 
class shopee_orderitems_model extends CI_Model
{
...
$num=count($arr["statuses"]);
  return $arr["statuses"][$num-1];
	...
	function insertOrderitems($data)
	{
	    ...
	    $this->db->insert('shopee_orderitems', $data); 
	...
	function insert_all($arr_data){
		//print_r($arr_data);
		$this->db->insert_batch('shopee_orderitems', $arr_data); 
	...
	function delete_by_keygen($keygen){
		$this->db->where('keygen',$keygen);
		$this->db->delete('shopee_orderitems');
	}
	...
	function delete_order_number($order_sn){
		$this->db->where('order_sn',$order_number);
		$this->db->delete('shopee_orderitems');
	}

	function select_order_item_shopee_last_null($id){
		$this->db->select('*');
		$this->db->from('shopee_orderitems');
		$this->db->where('OrderItemID',$id);
		$this->db->where('order_sn',NULL);
		$query = $this->db->get();
		return $query->row_array();
	}

	function select_order_item_shopee_last_null_bk(){
		$this->db->select('*');
		$this->db->from('shopee_orderitems');
		$this->db->where('order_sn',NULL);
		$this->db->order_by('OrderItemID','desc');
		$this->db->limit(1);
		$query = $this->db->get();
		return $query->row_array();
	}
