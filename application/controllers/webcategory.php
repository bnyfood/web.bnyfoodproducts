...
class Webcategory extends CI_Controller
{
	...
	function category_add_form(){
		...
		$this->view_util->load_view_main('webcategory/add_cat',$data,NULL,$arr_js,$arr_input,MENU_CONFIG_USERGROUP);
	...
	function category_add_action(){
		...
		$arr_res = $this->curl_bl->CallApi('POST','webcategory/category_add_action',$data_curl);
		//print_r($arr_res);
	...
	function category_edit_form(){
		...
		$arr_cat = $this->curl_bl->CallApi('GET','webcategory/get_cat/'.$parent_id);
		$arr_input = array(
			'title' => "Config Category"
		);
		...
		$this->view_util->load_view_main('webcategory/edit_cat',$data,NULL,$arr_js,$arr_input,MENU_CONFIG_USERGROUP);
	...
	function category_edit_action(){
		...
		$arr_res = $this->curl_bl->CallApi('POST','webcategory/category_edit_action',$data_curl);
		//print_r($arr_res);
		...
		redirect(base_url().'webcategory/manage_cat','refresh');
	...
	function category_del_action(){
		...
		$arr_res = $this->curl_bl->CallApi('GET','webcategory/category_del_action/'.$parent_id);
		//print_r($arr_res);
	...
	public function manage_cat()
	{
		...
		$sess_shop_id = $this->session->userdata(SESSION_PREFIX.'shop_id');
		$customer_type = $this->session->userdata(SESSION_PREFIX.'customer_type');
		$customer_code = $this->session->userdata(SESSION_PREFIX.'customer_code');
		$usergroup_id = $this->session->userdata(SESSION_PREFIX.'usergroup_id');
		...
		$id_shop = $this->session->userdata(SESSION_PREFIX.'id_shop_cat');

		//echo ">>>>>>>>>>>>>>".$id_shop;

		$arr_list_cats = $this->curl_bl->CallApi('GET','webcategory/build_cat/'.$id_shop.'/material');
		//print_r($arr_list_cats['Data']);
		$arr_cat_root = $this->curl_bl->CallApi('GET','webcategory/get_cat_root/'.$id_shop.'/material');
		...
		if($from_pop == 1){
			$this->view_util->load_view_blankpage('webcategory/manage_cat',$data,NULL,$arr_js,$arr_input,MENU_CONFIG_USERGROUP);
		}else{
			$this->view_util->load_view_main('webcategory/manage_cat',$data,NULL,$arr_js,$arr_input,MENU_CONFIG_USERGROUP);
		}
	...
	public function manage_cat_pop()
	{
		...
		$this->view_util->load_view_blankpage('webcategory/manage_cat_pop',$data,NULL,$arr_js,$arr_input,MENU_CONFIG_USERGROUP);
	...
	function category_add(){
		...
		if($is_add == 1){
			...
			redirect(base_url().'webcategory/manage_cat/'.$from_pop,'refresh');
		}else if($is_add == 2){
	...
	function category_get_by_id(){
		...
		$arr_cat = $this->curl_bl->CallApiNospi('GET','webcategory/get_cat/'.$parent_id);
	...
	function set_shop_search(){
		...
		//echo $id_shop;
		$this->session->set_userdata(SESSION_PREFIX.'id_shop_cat',$id_shop);
		redirect(base_url().'webcategory/manage_cat','refresh');
	}
	...
	function get_cat_list(){
		...
		//print_r($arr_user);
		$data = $this->selectbox_bl->make_list_webcat($arr_list_cats['Data']);
		$arr_data = array(
			'list_cat' => $data
		);
		echo json_encode($arr_data);
