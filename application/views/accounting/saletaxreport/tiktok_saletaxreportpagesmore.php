  ...
  <body style="background: #FFF;">
    <div class="col-picker no-print">
      ...
      <div id="col_checks">
        ...
        <label><input type="checkbox" data-toggle-col="orderno" checked> TTKOrderNumber</label>
        ...
        <label><input type="checkbox" data-toggle-col="tiktok"> ส่วนลด TikTok</label>
      ...
      <div style="color:#555;margin-top:4px;">มาตรฐาน (ส่งสรรพากร) = ลำดับ, วัน, ใบกำกับ, TTKOrderNumber, ผู้ซื้อ, เลขผู้เสียภาษี, มูลค่าสินค้า, ส่วนลดร้านค้า, ค่าขนส่ง, ราคาไม่รวม VAT, VAT, ราคารวม VAT — ไม่รวมส่วนลด TikTok / ส่วนลดรวม — แถบนี้ไม่ถูกพิมพ์</div>
    </div>
    <center>
      <?php
      if($validdata==0)
      ...
      <table class="table1 table-borderless">
        ...
        <thead>
          <tr>
              <th colspan="6" style="text-align: left;">เลขที่ใบกำกับภาษี: <?php echo "TTK".$order_start ."- TTK".$order_end; ?></th>
              <th colspan="2" style="text-align: right;"> วันที่: <?php echo date("Y/m/d");?></th>
          </tr>
          <tr>
            <th colspan="8"><center><h1>รายงานภาษีขาย(Tiktok)</h1></center></th>
          </tr>
          <tr>
      ...
      <table class="table2">
        ...
        <col data-col="invoice" style="width:16%">
        <col data-col="orderno" style="width:12%">
        <col data-col="buyer" style="width:7%">
        <col data-col="taxid" style="width:7%">
        <col data-col="price" style="width:8%">
        <col data-col="seller" style="width:8%">
        <col data-col="tiktok" style="width:8%">
        <col data-col="totaldisc" style="width:8%">
        <col data-col="ship" style="width:8%">
        ...
        <thead>
          <tr class="colhead">
            ...
            <th data-col="orderno" style="text-align:center">TTKOrderNumber</th>
            ...
            <th data-col="tiktok" style="text-align:right;line-height: initial;">ส่วนลด<br>TikTok</th>
      ...
      <?php } ?>
        <tbody>
          <?php
            ...
            if ($seller_disc == 0.0 && isset($row['seller_discount'])) {
              $seller_disc = floatval($row['seller_discount']);
            }
            ...
            if ($legacy_money && isset($row["priceVATincluded"])) {
              ...
              $priceBeforeVAT = isset($row["priceBeforeVAT"]) ? floatval($row["priceBeforeVAT"]) : ($ref_price / 1.07);
              $VAT = isset($row["VAT"]) ? floatval($row["VAT"]) : ($ref_price - $priceBeforeVAT);
            } else {
            ...
            $order_no = isset($row['order_id']) ? $row['order_id'] : (isset($row['order_sn']) ? $row['order_sn'] : '');
          ?>
          <tr class="data-row <?php echo ($row_runner % 2) ? 'zebra-odd' : 'zebra-even'; ?>">
            ...
            <td data-col="orderno" class="transacdetail" style="text-align:center"><?php echo $order_no;?></td>
            ...
            <td data-col="tiktok" class="transacdetail"><?php echo number_format($voucher_platform,2,".",",");?></td>
          ...
          if(($page_runner+1)==$totalpages)
            ...
            <tr class="total-row">
              ...
              <td data-col="tiktok" class="conclution" ><div style="text-align: right;"><?php echo number_format($page_voucher_platform,2,".",","); ?></div></td>
...
<script>
  var COL_STORE = "tiktok_tax_report_cols_v2";
  var STANDARD_ON = ["no","date","invoice","orderno","buyer","taxid","price","seller","ship","vatexcl","vat","vatincl"];
  var ALL_COLS = ["no","date","invoice","orderno","buyer","taxid","price","seller","tiktok","totaldisc","ship","vatexcl","vat","vatincl"];
