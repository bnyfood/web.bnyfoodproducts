    ...
    <center>
      ...
      ?>
        ...
        <thead>
          ...
          <tr>
            <th colspan="8"><center><h1>รายงานใบลดหนี้(Shopee) รายออเดอร์ประจำเดือน</h1></center></th>
          </tr>  
          <tr>
        ...
        <thead>   
            <tr class="colhead">    
                ...
                <th style="text-align:center">ShoOrderNumber</th>
        ...
        <tbody>                                   
          <tr class="data-row <?php echo ($row_runner % 2) ? 'zebra-odd' : 'zebra-even'; ?>">
            ...
            <td class="transacdetail" style="text-align:center">
              <?php
                $cn_day = date_create($row["updated_at"]);
                $cn_mdy = $cn_day ? $cn_day->format("m/d/Y") : $row["updated_at"];
                $cn_range = str_replace(array('/', ' ', '-'), array('sl', 'sp', 'hp'), $cn_mdy.' - '.$cn_mdy);
                $order_sn = isset($row['order_number']) ? rawurlencode($row['order_number']) : 'none';
                $cn_doc_url = base_url()."accounting/creditnote/creditnote_make/1/".$order_sn."/".$cn_range."/original/4";
              ?>
              <a href="<?php echo $cn_doc_url; ?>" style="text-decoration: none" class="morecn" id="<?php echo $cn_doc_url; ?>" rel="nofollow"><?php echo $row["cncode"];?></a>
            </td>
      ...
      <div id="report_daily" class="report-panel report-daily">
        <?php
          $daily_orders = !empty($shopee_orders_daily) ? $shopee_orders_daily : array();
          $d_price = 0;
          $d_vat = 0;
          $d_incl = 0;
          $d_n = 0;
        ...
        <table class="table1 table-borderless">
          <thead>
            ...
            <tr>
              <th colspan="8"><center><h1>รายงานใบลดหนี้(Shopee) สรุปรวมรายวันทั้งเดือน</h1></center></th>
            </tr>
            <tr>
        ...
        <table class="table2">
          ...
          <tbody>
            <?php foreach($daily_orders as $drow) {
              $d_n++;
              if (isset($drow['vatincl'])) {
                $tax = array(
                  'vatexcl' => $drow['vatexcl'],
                  'vat' => $drow['vat'],
                  'vatincl' => $drow['vatincl']
                );
              } else {
