  ...
  <head>
    ...
    <style>
     ...
     .transacdetail{
      ...
      .table2 tbody tr.data-row.row-returned td {
      ...
      @media print {
        ...
        body.print-daily .report-orders { display: none !important; }
        body.print-daily .report-daily { display: block !important; }
        body.print-orders .report-daily { display: none !important; }
        body.print-orders .report-orders { display: block !important; }
        @page {
          size: A4 landscape;
          margin: 7mm 6mm;
        }
    ...
    <center>                                                
      ...
      ?>
        ...
        (วันที่จาก <?php echo $start_date_date->format("d/m/Y"); ?> ถึง <?php echo $end_date_date->format("d/m/Y"); ?>) </th>
          ...
          <tr>
            <th colspan="8"><center><h1>รายงานใบลดหนี้(Lazada) รายออเดอร์ประจำเดือน</h1></center></th>
          </tr>  
          <tr>
        ...
        <thead>   
            ...
            <tbody>                     
              <tr class="data-row <?php
                if (($row["cn_status"] == 0) && ($row["status2"] == "returned")) {
                  echo 'row-returned';
                } else {
                  echo ($row_runner % 2) ? 'zebra-odd' : 'zebra-even';
                }
              ?>">
      ...
      <div id="report_daily" class="report-panel report-daily">
        <?php
          $daily_orders = !empty($lazada_orders_daily) ? $lazada_orders_daily : array();
          $d_price = 0;
          $d_vat = 0;
          $d_incl = 0;
          $d_n = 0;
        ...
        <table class="table1 table-borderless">
          <thead>
            ...
            <tr>
              <th colspan="8"><center><h1>รายงานใบลดหนี้(Lazada) สรุปรวมรายวันทั้งเดือน</h1></center></th>
            </tr>
            <tr>
        ...
        <table class="table2">
          ...
          <tbody>
            <?php foreach($daily_orders as $drow) {
              $d_n++;
              $tax = $this->order_util->cn_tax_amounts($drow, false);
              $d_price += $tax['vatexcl'];
              $d_vat += $tax['vat'];
              $d_incl += $tax['vatincl'];
            ?>
