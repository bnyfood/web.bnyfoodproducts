    ...
    <center>                                                
      ...
      ?>
        ...
        (วันที่จาก <?php echo $start_date_date->format("d/m/Y"); ?> ถึง <?php echo $end_date_date->format("d/m/Y"); ?>) </th>
          ...
          <tr>
            <th colspan="8"><center><h1>รายงานใบลดหนี้(Tiktok) สรุปรวมรายวัน</h1></center></th>
          </tr>  
          <tr>
            ...
            <tbody>                     
              <tr class="data-row <?php echo ($row_runner % 2) ? 'zebra-odd' : 'zebra-even'; ?>">
                 ...
                 <td class="transacdetail" style="text-align:center">
                  <a href="#" style="text-decoration: none" id="<?php echo base_url()."accounting/creditreport/tik_make_cn_by_date/".$row["updated_at"]?>" class="morecn" rel="nofollow"><?php echo $row["cncode"];?></a>
                 </td>
                 <?php
...
<script>
  $("a.morecn").on("click", function() {
    ...
    window.open(share_link, "_blank", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, copyhistory=no,top=80,left=80,width=1200,height=800");
    return false;
  });
  ...
  $("#cols_orders").on("click", function() {
    var url = <?php echo json_encode(base_url()."accounting/creditreport/tik_make_cn_month/".$start_date."/".$end_date); ?>;
