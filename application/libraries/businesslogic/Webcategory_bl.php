...
class Webcategory_bl{
	...
	public function __construct(){
		...
		$this->CI->load->model('web_category_model');
	}
...
function get_cat_sud_by_id($parentcategory,$shop_id){
	...
	function getsub_cat_by_id($parentcategory,$shop_id){
		...
		$sub_arr_cats = $this->CI->web_category_model->select_by_parent($parentcategory,$shop_id);

		if(!empty($sub_arr_cats)){
			foreach($sub_arr_cats as $sub_arr_cat){
				//echo ">>>".$sub_arr_cat['name']."<br>";
				$arr_cat = $this->CI->web_category_model->select_by_id($sub_arr_cat['ProductCategoryID']);
	...
	function get_cat_by_parent($class,$parent,$shop_id){
		$main_arr_cats = $this->CI->web_category_model->select_by_parent($parent,$shop_id);
				...
				if($cnt > 0){
					if(!empty($arr_sub_cat[$i][$main_arr_cat['ProductCategoryID']])){
						...
						for($j=0;$j<=$cnt_sub_last;$j++){
							...
							$arr_cat_sub_last = $this->CI->web_category_model->select_by_id($arr_group_sub[$j]);
	...
	function bulid_cat_by_parent($class,$parentcategory,$shop_id){
	$this->arr_sub_cat = [];
		//$main_arr_cat = $this->CI->categories_model->select_by_catid($cat_id);
		$main_cat = array();
		$num= 0;
		$num_lv = 0;
			...
			$sub_cat = $this->bulid_sub_cat_by_parent($class,$parentcategory,$shop_id,$num_lv);
			//print_r($sub_cat);
			$main_cat[$num] = $sub_cat;
			$num = $num+1;
	...
	function bulid_sub_cat_by_parent($class,$parentcategory,$shop_id,$num_lv){
		
		$sub_arr_cats = $this->CI->web_category_model->select_by_class_parent($class,$parentcategory,$shop_id);
		//print_r($sub_arr_cats);
		if(!empty($sub_arr_cats)){
			$num_lv = $num_lv+1;
			foreach($sub_arr_cats as $sub_arr_cat){
				//echo $num_lv.">>>".$sub_arr_cat['Title']."<br>";
				$arr_cat = $this->CI->web_category_model->select_by_id($sub_arr_cat['web_category_id']);
				$arr_cat['level'] = $num_lv;
				//if($parentcategory <> 0){
					array_push($this->arr_sub_cat,$arr_cat);
				//}
				$this->bulid_sub_cat_by_parent($class,$sub_arr_cat['web_category_id'],$shop_id,$num_lv);
